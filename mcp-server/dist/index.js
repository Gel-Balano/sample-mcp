...
server.registerResource("generated-cars-list", "cars://", {
...
server.registerResource("shops", "shops://", {
    title: "Shops Data",
...
server.registerResource("customers", "customers://", {
...
server.registerResource("shop-customers", // Resource name
new ResourceTemplate("shop-customers://{shopId}", { list: undefined }), // URI template with parameters
{
    ...
    if (shopId === '{shopId}') {
        const availableShops = shops.map((s) => ({ id: s.id, name: s.name, type: s.type }));
        return {
            contents: [{
    ...
    if (isNaN(shopIdNum)) {
        const availableShops = shops.map((s) => ({ id: s.id, name: s.name }));
    ...
    const shopCustomers = customers.filter((c) => c.favoriteShopId === shopIdNum);
    console.log('Found customers:', shopCustomers.length);
    if (shopCustomers.length === 0) {
        const availableShops = shops.map((s) => ({ id: s.id, name: s.name }));
        throw new Error(`No customers found for shop ID ${shopId}. Valid shop IDs are: ${JSON.stringify(availableShops.map((s) => s.id))}`);
    }
    const shop = shops.find((s) => s.id === shopIdNum);
    return {
        contents: [{
                uri: `shop-customers://${shopId}`,
                text: JSON.stringify({
...
server.registerTool("create_customer", {
    ...
    try {
        ...
        if (!shop) {
            throw new Error(`Shop with ID ${shop_id} not found. Available shop IDs: ${shops.map((s) => s.id).join(', ')}`);
        }
...
server.registerTool("compute_customer_expenses", {
    ...
    try {
        ...
        if (!customer) {
            throw new Error(`Customer with ID ${customer_id} not found. Available customer IDs: ${customers.map((c) => c.id).join(', ')}`);
        }
        // Filter transactions for this customer
        const customerTransactions = transactions.filter((t) => t.customerId === customer_id);
        ...
        const totalExpenses = customerTransactions.reduce((sum, transaction) => sum + transaction.amount, 0);
        // Group transactions by shop
        const expensesByShop = customerTransactions.reduce((acc, transaction) => {
        ...
        const summary = {
            ...
            recentTransactions: customerTransactions.slice(0, 5).map((t) => ({
                id: t.id,
                amount: t.amount,
                date: t.date,
                description: t.description,
                shopName: shops.find((s) => s.id === t.shopId)?.name || `Shop ${t.shopId}`
            }))
        };
        return {
            content: [{
