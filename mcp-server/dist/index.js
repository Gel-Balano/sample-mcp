...
server.registerResource("customers", "customers", {
    ...
    return {
        contents: customers.map((customer) => ({
...
server.registerResource("shop-customers", "shop-customers", {
    ...
    const shopCustomers = customers.filter((c) => c.favoriteShopId === parseInt(shopId));
    return {
        contents: shopCustomers.map((customer) => ({
            uri: `customers://${customer.id}`,
            text: JSON.stringify(customer, null, 2)
        }))
    };
